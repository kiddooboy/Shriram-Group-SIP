        ...
        <AnimatePresence mode="wait">
          {phase === 'empid' && (
            <motion.div key="empid" className="flex-1 flex flex-col" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }}>
              ...
              <p className="text-smf-muted text-sm mb-8">Single sign-on via Shriram Group SSO.</p>

              <div className="cred-card px-5 py-5 mb-3 bg-white border border-smf-line">
                ...
                <input
                  ...
                  placeholder="SF10042"
                  className="w-full bg-transparent text-smf-teal-dark text-2xl font-bold tracking-[0.12em] placeholder-smf-muted/30 focus:outline-none font-mono"
                />
              </div>
              ...
              <div className="cred-row px-4 py-3 mb-7 bg-white border border-smf-line">
                <p className="text-smf-muted text-[12px]">
                  <span className="text-smf-amber font-semibold">Demo IDs:</span> SF10042 · SF20183 · SF30561 · SF40892 · SF50234 · DEMO
                </p>
              </div>
          ...
          {phase === 'otp' && (
            <motion.div key="otp" className="flex-1 flex flex-col" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }}>
              ...
              <p className="text-smf-muted text-sm mb-8">Sent to your Aadhaar-linked mobile · +91 ••••• ••789</p>
              ...
              <div className="flex gap-2 justify-between mb-4">
                {otp.map((digit, idx) => (
                  <input
                    ...
                    className="w-[52px] h-[60px] bg-white border border-smf-line rounded-2xl text-smf-teal-dark text-2xl font-bold text-center focus:outline-none focus:border-smf-teal transition-colors"
                  />
                ))}
              </div>
              <p className="text-smf-muted text-[12px] mb-8 font-semibold">Demo — enter any 6 digits.</p>
          ...
          {phase === 'verifying' && (
            <motion.div key="verifying" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center pt-24">
              <div className="relative mb-6">
                <div className="w-16 h-16 rounded-full border-[3px] border-smf-teal/15 border-t-smf-teal animate-spin" />
                <ShieldCheck className="w-6 h-6 text-smf-teal absolute inset-0 m-auto" />
              </div>
              <p className="text-smf-teal-dark font-semibold">Verifying with UIDAI</p>
              <p className="text-smf-muted text-[13px] mt-1">Confirming your identity…</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
